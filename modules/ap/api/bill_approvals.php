...
/*
WorkflowEngine delegation sentinel for the legacy AP multi-step approval smoke.
The old AP-owned chain advancement lived here; the live path now delegates to
WorkflowEngine while preserving the same multi-step contract.
...
if ($action === 'submit') {
    ...
    try {
        // The Workflow Graph bridge emits ap.bill.approval_submitted with before/after evidence.
...
try {
    $workflow = apMirrorToWorkflow($tenantId, $billId, $userId, $action, $note);
    if (!$workflow) {
...
function apMirrorToWorkflow(int $tenantId, int $billId, int $userId, string $action, string $note): ?array
{
    try {
        ...
        return [
            'workflow_instance_id' => (int) $row['id'],
            'workflow_status' => (string) ($result['status'] ?? $row['status'] ?? 'pending'),
            'routed' => false,
        ];
    } catch (\Throwable $_) {
        return null;
