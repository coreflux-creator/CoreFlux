 ...
 *
 *   GET    /api/accounting/journal_entries
 *   GET    /api/accounting/journal_entries?id=N
 *   GET    /api/accounting/journal_entries?action=trial_balance&as_of=YYYY-MM-DD[&entity_id=]
 *   POST   /api/accounting/journal_entries
 *   POST   /api/accounting/journal_entries?action=draft
 *   POST   /api/accounting/journal_entries?action=submit&id=N
 *   POST   /api/accounting/journal_entries?action=approve&id=N
 *   POST   /api/accounting/journal_entries?action=reject&id=N
 *   POST   /api/accounting/journal_entries?action=post&id=N
 *   POST   /api/accounting/journal_entries?action=reverse&id=N
 *   POST   /api/accounting/journal_entries?action=void&id=N
 */
...
function accountingJeWorkflowApiError(\Throwable $e): void
{
    $msg = $e->getMessage();
    $code = (
        stripos($msg, 'Separation of duties') !== false
        || stripos($msg, 'not an approver') !== false
        || stripos($msg, 'not authorized') !== false
    ) ? 403 : 409;
    api_error($msg, $code);
}
...
if ($method === 'GET') {
    ...
    if (!empty($_GET['approval_state'])) { $where[] = 'je.approval_state = :astate'; $params['astate'] = $_GET['approval_state']; }
    ...
    $rows = scopedQuery(
        'SELECT DISTINCT je.id, je.je_number, je.posting_date, je.entity_id, je.period_id, je.source_module,
                je.source_ref_type, je.source_ref_id, je.status, je.approval_state, je.workflow_instance_id,
                je.submitted_at, je.approved_at, je.currency, je.total_debit, je.total_credit, je.memo, je.posted_at
...
if ($method === 'POST' && $action === 'reverse') {
    ...
    try {
        $res = accountingReverseJe($tid, $id, $reason, $actorUserId);
...
if ($method === 'POST' && $action === 'submit') {
    rbac_legacy_require($user, 'accounting.je.submit');
    $id = (int) ($_GET['id'] ?? 0);
    if ($id <= 0) api_error('id required', 422);
    $workflowId = accountingJeWorkflowStart($tid, $id, $actorUserId);
    if (!$workflowId) api_error('Could not submit journal entry for approval', 409);
    api_ok([
        'je_id' => $id,
...
if ($method === 'POST' && in_array($action, ['approve', 'reject'], true)) {
    rbac_legacy_require($user, 'accounting.je.approve');
    $id = (int) ($_GET['id'] ?? 0);
    if ($id <= 0) api_error('id required', 422);
    $body = api_json_body();
    try {
        $workflow = accountingJeWorkflowAct(
            $tid,
            $id,
            (int) ($actorUserId ?? 0),
            $action,
            trim((string) ($body['note'] ?? $body['reason'] ?? '')) ?: null
        );
    } catch (\Throwable $e) {
        accountingJeWorkflowApiError($e);
    }
    api_ok([
        'je_id' => $id,
        'approval_state' => $workflow['journal_entry']['approval_state'] ?? null,
        'approved' => (bool) ($workflow['approved'] ?? false),
        'rejected' => (bool) ($workflow['rejected'] ?? false),
        'workflow_instance_id' => $workflow['instance']['id'] ?? null,
        'workflow_status' => $workflow['instance']['status'] ?? null,
        'journal_entry' => $workflow['journal_entry'] ?? null,
    ]);
}

if ($method === 'POST' && $action === 'post' && (int) ($_GET['id'] ?? 0) > 0) {
    rbac_legacy_require($user, 'accounting.je.post');
    $id = (int) $_GET['id'];
    try {
        $res = accountingPostApprovedDraftJe($tid, $id, $actorUserId);
    } catch (\Throwable $e) {
        accountingJeWorkflowApiError($e);
    }
    accountingAudit('accounting.je.posted', [
        ...
        'source' => 'workflow_approved_draft',
        'workflow_instance_id' => $res['workflow_instance_id'] ?? null,
    ], $res['je_id']);
    api_ok($res);
}
...
if ($method === 'POST' && $action === 'void') {
    ...
    $reason = trim((string) ($body['reason'] ?? ''));
    try {
        $res = accountingVoidDraftJe($tid, $id, $reason, $actorUserId);
    } catch (\Throwable $e) { api_error($e->getMessage(), 409); }
    accountingAudit('accounting.je.voided', ['je_id' => $id, 'reason' => $reason], $id);
    api_ok($res);
}
...
if ($method === 'POST' && $action === 'draft') {
    ...
    try {
        $res = accountingPostJe($tid, $body, $actorUserId, false);
    } catch (\Throwable $e) { api_error($e->getMessage(), 422); }
    accountingAudit('accounting.je.drafted', [
        'je_id' => $res['je_id'],
        'je_number' => $res['je_number'],
        'total' => $res['total_debit'],
        'source' => $body['source_module'],
        'replay' => $res['idempotent_replay'],
    ], $res['je_id']);
    api_ok($res, 201);
}

if ($method === 'POST' && ($action === '' || $action === 'post')) {
    rbac_legacy_require($user, 'accounting.je.post');
    $body = api_json_body();
    api_require_fields($body, ['posting_date', 'lines']);
    $body['source_module'] = $body['source_module'] ?? 'manual';
    try {
        $res = accountingPostJe($tid, $body, $actorUserId, true);
    } catch (\Throwable $e) { api_error($e->getMessage(), 422); }
    accountingAudit('accounting.je.posted', [
        'je_id' => $res['je_id'],
        'je_number' => $res['je_number'],
