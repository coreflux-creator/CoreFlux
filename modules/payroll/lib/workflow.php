<?php
/** Payroll run bridge to the shared WorkflowGraph approval engine. */
...
function payrollRunWorkflowContext(array $run, ?int $actorUserId = null): array {
    ...
    foreach (['created_by_user_id', 'computed_by_user_id'] as $key) {
        if (isset($run[$key]) && (int) $run[$key] > 0) $context[$key] = (int) $run[$key];
    }
...
function payrollRunWorkflowPayload(array $run, ?int $actorUserId = null): array {
    ...
    return [
        ...
        'body' => sprintf(
            ...
            $runId,
            (string) ($run['pay_date'] ?? ''),
            number_format(((int) ($run['net_total_cents'] ?? 0)) / 100, 2),
...
function payrollRunWorkflowStart(int $tenantId, int $runId, ?int $actorUserId = null): ?int {
    ...
    try {
        ...
        workflowEnsureDefinition($tenantId, $defKey, 'payroll_run', 'Payroll run approval', payrollRunWorkflowSteps($runId));
        ...
        $pdo = getDB();
        $pdo->prepare(
            'UPDATE payroll_runs SET workflow_instance_id = :w, updated_at = NOW()
        ...
        $pdo->prepare(
            'UPDATE workflow_instances SET payload_json = :payload, last_activity_at = NOW()
...
function payrollRunWorkflowCancelPending(int $tenantId, int $runId, ?int $actorUserId = null, string $reason = 'recompute'): int {
    ...
    $in = implode(',', $ids);
    $pdo->exec(
        "UPDATE workflow_instances SET status = 'cancelled', completed_at = NOW(), last_activity_at = NOW()
...
function payrollRunWorkflowAct(int $tenantId, array $run, int $userId, string $action, ?string $note = null): array {
    ...
    try {
        ...
        if ($instanceId > 0) {
            $check = $pdo->prepare(
                "SELECT id FROM workflow_instances WHERE tenant_id = :t AND id = :id AND status = 'pending'"
            );
        ...
        if ($instanceId <= 0) $instanceId = (int) (payrollRunWorkflowStart($tenantId, $runId, null) ?? 0);
        if ($instanceId <= 0) throw new \RuntimeException('Could not start payroll approval workflow');
        ...
        return ['applied' => true, 'approved' => $approved, 'instance' => $instance, 'run' => $updated];
    } catch (\Throwable $e) {
