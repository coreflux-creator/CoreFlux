...
--
-- v2 (2026-02): fallback no-ops use `DO 0` instead of `SELECT '…' AS note`
-- so the unfetched result set doesn't break the next PDO query with
...
;
SET @sql := IF(@te_exists = 1 AND @col = 0, 'ALTER TABLE time_entries ADD COLUMN timesheet_id BIGINT UNSIGNED NULL AFTER period_id, ADD INDEX idx_te_timesheet (timesheet_id)', 'DO 0')
;
...
;
SET @sql := IF(@te_exists = 1 AND @col = 0, "ALTER TABLE time_entries ADD COLUMN hour_type ENUM('regular','overtime','doubletime','holiday','pto','sick','bereavement','unpaid','nonbillable') NOT NULL DEFAULT 'regular' AFTER category", 'DO 0')
;
PREPARE stmt FROM @sql
;
EXECUTE stmt
...
;
SET @sql := IF(@te_exists = 1 AND @col = 0, "ALTER TABLE time_entries ADD COLUMN billable TINYINT(1) NOT NULL DEFAULT 1 AFTER hour_type, ADD COLUMN payable TINYINT(1) NOT NULL DEFAULT 1 AFTER billable", 'DO 0')
;
PREPARE stmt FROM @sql
;
EXECUTE stmt
