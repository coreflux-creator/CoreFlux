...
if ($method === 'GET' && $action === 'week') {
    rbac_legacy_require_any($user, ['time.view', 'staffing.time.view']);
...
if ($method === 'GET' && $action === 'prefill_from_last_week') {
    rbac_legacy_require_any($user, ['time.entry.create', 'staffing.time.create']);
    $personId    = (int) ($_GET['person_id']    ?? ($user['person_id'] ?? 0));
    $periodStart = (string) ($_GET['period_start'] ?? '');
    $periodEnd   = (string) ($_GET['period_end']   ?? '');
    if ($personId <= 0)                api_error('person_id required', 422);
...
if ($method === 'GET' && $action === 'week_economics') {
    rbac_legacy_require_any($user, ['reports.view', 'staffing.reports.view']);
    $personId    = (int) ($_GET['person_id']    ?? ($user['person_id'] ?? 0));
    $periodStart = (string) ($_GET['period_start'] ?? '');
    $periodEnd   = (string) ($_GET['period_end']   ?? '');
    if ($personId <= 0)                api_error('person_id required', 422);
...
if ($method === 'GET' && $action === 'approved_hours_ready') {
    rbac_legacy_require_any($user, ['reports.view', 'staffing.view']);
...
if ($method === 'POST' && in_array($action, ['submit','approve','reject'], true)) {
    if ($action === 'submit') {
        rbac_legacy_require_any($user, ['time.entry.create', 'staffing.time.submit']);
    } elseif ($action === 'approve') {
        rbac_legacy_require_any($user, ['time.approve', 'staffing.time.approve']);
    } else {
        rbac_legacy_require_any($user, ['time.reject', 'staffing.time.reject']);
    }
    $body = api_json_body();
