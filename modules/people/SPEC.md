...
| `resume_doc_id` | BIGINT NULL FK→`people_documents.id` | latest resume |
...
| `file_path` | VARCHAR(512) | object storage path |
...
| `id` | BIGINT PK | |
| `person_id` | BIGINT FK | |
| `stage` | ENUM('sourced','screened','submitted','interview','offer','placed','bench','terminated','rejected') | |
| `entered_at` | DATETIME | |
...
1. **Object storage** for documents — stay on Cloudways disk, or use S3 / Backblaze? (Affects `people_documents.file_path` semantics.)
2. **Encryption at rest** for banking — application-level (PHP openssl with KMS key) or DB-level only? Recommend application-level.
3. **PII access logging** — do tenant_admins get a self-serve PII audit report, or only master_admin?
4. **Pipeline stages** — fixed enum above, or tenant-customizable? Default: fixed enum + tenant-defined sub-statuses.
5. **Person merge** — needed for MVP, or deferred?
