...
function StoredAssignmentDetails({ row }) {
  ...
  const currentGraphPairs = [
    ...
    ['Rate rows', row.current_graph?.rates_count ?? row.current_graph?.rates?.length ?? 0],
    ['Client/vendor chain rows', row.current_graph?.chain_count ?? row.current_graph?.chain?.length ?? 0],
    ['Corp/vendor records', row.current_graph?.corp_count ?? row.current_graph?.corp?.length ?? 0],
    ['Commission rows', row.current_graph?.commissions_count ?? row.current_graph?.commissions?.length ?? 0],
    ['Referral rows', row.current_graph?.referrals_count ?? row.current_graph?.referrals?.length ?? 0],
    ['Economic parties', row.current_graph?.economic_parties_count ?? row.current_graph?.economic_parties?.length ?? 0],
