...
function shapePlacement(row: any): any {
  ...
  return {
    ...
    engagementType: row.engagement_type === '1099' ? 'contract_1099' : (row.engagement_type ?? null),
    remotePolicy: row.remote_policy ?? null,
    worksiteState: row.worksite_state ?? null,
    worksiteCountry: row.worksite_country ?? null,
    staffingJobId: row.staffing_job_id != null ? String(row.staffing_job_id) : null,
    branch: row.branch ?? null,
    serviceLine: row.service_line ?? null,
    workersCompClass: row.workers_comp_class ?? null,
    department: row.department ?? null,
    costCenter: row.cost_center ?? null,
    accountingEntityId: row.accounting_entity_id != null ? String(row.accounting_entity_id) : null,
