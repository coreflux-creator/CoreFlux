...
| 2026-02-XX | **Every module that owns a primary entity (people, placements, clients, vendors, time entries, etc.) MUST expose CSV import.** Built on a Core platform primitive at `/core/CsvImportService.php`. Standard contract: downloadable template per entity, multipart upload → dry-run preview (validation + error rows + duplicate detection) → user confirms → committed import with audit log. Modules MUST NOT roll their own importers. Applies retroactively (People retrofitted). | User chat (this fork) |
| 2026-02 (spec re-audit) | **Treasury sweep destination has a UI** (not CLI-only). Divergence alerts surface as in-app notifications, not just email/log. | User chat (this fork) |
| 2026-02 (spec re-audit) | **Time module grace period for bundle corrections IS required.** Reverses earlier "no grace period on consumed bundles" rule. Operator confirmed: "Time can have a grace period. In fact it needs it." Bundle accruals must be reversible / supersedable within a configurable grace window even after downstream postings. | User chat (this fork) |
| 2026-02 (spec re-audit) | **Accounting close packets must be wired into the actual close workflow** (not just data files). Owners + due-dates + blocking gates active. | User chat (this fork) |
| 2026-02 (spec re-audit) | **Consolidation supports both equity method and proportionate consolidation**, plus CTA (Cumulative Translation Adjustment) postings. v1.0 scope expanded. | User chat (this fork) |
| 2026-02 (spec re-audit) | **Multi-level approval chain must actually fire**, not just be stored. Rules evaluate in `level` order; chain enforced at API layer. | User chat (this fork) |
| 2026-02 (spec re-audit) | **AP three-way match is a HARD rule, not a soft warn.** Reverses earlier "soft warn" decision. Bills that fail 3-way match cannot be posted/paid without explicit override + reason. | User chat (this fork) |
| 2026-02 (spec re-audit) | **AP FOUR-way match for "paid when paid" terms.** When a vendor placement carries "paid when paid" terms, the fourth gate is: matching client payment must be received for the same hours before vendor payment is releasable. Links time bundle → AR invoice → AR payment → AP bill → AP payment. | User chat (this fork) |
| 2026-02 (spec re-audit) | **Customer portal view = backlogged** (no longer Phase A MVP for billing). | User chat (this fork) |
| 2026-02 (spec re-audit) | **Gusto + QuickBooks integrations elevated to URGENT P0.** Top of next sequence. | User chat (this fork) |
| 2026-02 (spec re-audit) | **Mercury webhooks remain pending P1.** | User chat (this fork) |
| 2026-02 (spec re-audit) | **Reports must be enhanced: more visual, sharp, responsive, with click-to-drill-down on every metric.** Current 5 reports get a UX/IA overhaul before adding new ones. | User chat (this fork) |
