...
| 2026-02-XX | **Accounting module = GL engine of record** (chart of accounts, journal entries immutable, period close, trial balance/P&L/BS/cash flow, bank rec, multi-currency at JE level, QB/Xero round-trip). Subledgers (Billing/AP/Payroll) post into Accounting via `/api/accounting/je/from-payload`. Posting always accrual; cash-basis is reporting toggle. | User chat (this fork) |
