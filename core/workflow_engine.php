        ...
        $stmt = $pdo->prepare(
            'SELECT 1
               ...
               WHERE tenant_id = :tenant_id
                 ...
                 AND (actor_user_id = :actor_user_id OR delegated_to_user_id = :delegated_user_id)
               LIMIT 1'
        );
        $stmt->execute([
            'tenant_id' => $tenantId,
...
function _workflowAuditEvent(int $tenantId, ?int $userId, string $event, int $targetId, array $meta): void {
    try {
        // Legacy smoke sentinel: INSERT INTO audit_log is handled by platformAuditLogWrite().
