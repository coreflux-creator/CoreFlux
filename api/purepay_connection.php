...
$keyScope = (string) ($body['key_scope'] ?? 'read_only');
if ($apiKey === '') api_error('api_key required', 422);
if (strlen($apiKey) < 12) api_error('api_key looks invalid (too short)', 422);
try { $result = purepayStoreConnection($tenantId, $apiKey, $label, $user['id'] ?? null, $keyScope); }
...
ppAudit('purepay.connection.connected', ['label'=>$label, 'key_scope'=>$keyScope], $tenantId, $user['id'] ?? null);
api_ok(['ok'=>true] + $result);
