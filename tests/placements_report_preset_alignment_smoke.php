...
$assert('expiring report resolves shared placement preset', str_contains($api, "reportBuilderPresetGet('placements.expiring_soon'"));
...
$assert('active-by-client aggregate remains explicitly isolated', str_contains($api, "type === 'active_by_client'") && str_contains($api, 'GROUP BY end_client_name'));
...
$assert('report builder docs explain module preset adapter', str_contains($docs, 'placements.expiring_soon') && str_contains($docs, 'source=module_preset'));
$assert('alignment docs identify aggregate gap', str_contains($alignment, 'Placement Active by Client remains a legacy aggregate adapter'));
