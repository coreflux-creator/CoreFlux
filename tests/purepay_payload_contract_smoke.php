...
$GLOBALS['__purepay_transport'] = function(string $method,string $url,array $headers,?string $body) use (&$requests): array {
    ...
    if ($method === 'GET' && str_ends_with($url, '/vendors/ven_1')) return ['status'=>200,'body'=>'{"vendor":{"id":"ven_1","display_name":"Acme","email":"pay@acme.test"}}'];
    if ($method === 'GET' && str_ends_with($url, '/bills')) return ['status'=>200,'body'=>'{"bills":[{"id":"bill_1"}]}'];
    if ($method === 'GET' && str_ends_with($url, '/bills/bill_1')) return ['status'=>200,'body'=>'{"bill":{"id":"bill_1"}}'];
    if ($method === 'GET' && str_ends_with($url, '/payments')) return ['status'=>200,'body'=>'{"payments":[{"id":"pmt_1","bill_id":"bill_1"}]}'];
    if ($method === 'GET' && str_ends_with($url, '/payments/pmt_1')) return ['status'=>200,'body'=>'{"payment":{"id":"pmt_1","bill_id":"bill_1"}}'];
    if ($method === 'GET' && str_ends_with($url, '/wallet')) return ['status'=>200,'body'=>'{"balance_cents":5000}'];
    return ['status'=>404,'body'=>'{"detail":"not found"}'];
};

$vendorPayload=['display_name'=>'Acme','email'=>'pay@acme.test','relationship'=>'vendor'];
...
$vendorKey=purepayIdempotencyKey(7,'vendor','ap_vendor:4');
$billKey=purepayIdempotencyKey(7,'bill','ap_payment:9');
$payKey=purepayIdempotencyKey(7,'pay','ap_payment:9');
$check('vendor create id extracted', purepayResourceId(purepayCreateVendor('pk_test', $vendorPayload, $vendorKey), ['vendor']) === 'ven_1');
$check('bill create id extracted', purepayResourceId(purepayCreateBill('pk_test', $billPayload, $billKey), ['bill']) === 'bill_1');
$check('pay create id extracted', purepayResourceId(purepayPayBill('pk_test','bill_1',$payPayload,$payKey), ['payment']) === 'pmt_1');
$check('vendor list normalizes', count(purepayCollection(purepayListVendors('pk_test'), ['vendors'])) === 1);
$check('vendor detail is addressable', purepayResourceId(purepayGetVendor('pk_test','ven_1'), ['vendor']) === 'ven_1');
...
$check('bill detail is addressable', purepayResourceId(purepayGetBill('pk_test','bill_1'), ['bill']) === 'bill_1');
$check('payment detail is addressable', purepayResourceId(purepayGetPayment('pk_test','pmt_1'), ['payment']) === 'pmt_1');
...
$check('every POST carries an idempotency key', !array_filter($requests, static fn(array $r): bool => $r['method']==='POST' && !array_filter($r['headers'], static fn(string $h): bool => str_starts_with($h,'Idempotency-Key: '))));
$check('bill and payment keys are distinct and stable', $billKey !== $payKey && $payKey === purepayIdempotencyKey(7,'pay','ap_payment:9'));
...
$check('driver re-GETs bills after create', str_contains($driver,'purepayGetBill($apiKey, $billId)'));
$check('driver re-GETs payments after pay', str_contains($driver,'purepayGetPayment($apiKey, $paymentId)'));
...
try {
    purepayCreateBill('pk_test',$billPayload,$billKey);
...
try {
    purepayPayBill('pk_test','bill_1',$payPayload,$payKey);
