...
foreach ($endpoints as $name => $rel) {
    ...
    $a("{$name} requires RBAC permission",       str_contains($body, "ai_suggest_map'")
        && preg_match('/ai_suggest_map.*?(?:api_require_legacy_permission|rbac_legacy_require|timeCsvRequireUpload)/s', $body));
