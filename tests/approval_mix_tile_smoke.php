...
$src = (string) file_get_contents($ROOT . '/modules/time/api/approval_mix.php');
...
foreach (['manual', 'tokenized_client_email', 'bulk_pre_approved', 'external_email'] as $ch) {
    $a("channel '{$ch}' present in pre-seed",
       str_contains($src, "'manual', 'tokenized_client_email', 'bulk_pre_approved', 'external_email'") || str_contains($src, "'{$ch}'"));
}
...
echo "\n8. PHP syntax\n";
$out = []; $rc = 0;
exec('php -l ' . escapeshellarg($ROOT . '/modules/time/api/approval_mix.php') . ' 2>&1', $out, $rc);
