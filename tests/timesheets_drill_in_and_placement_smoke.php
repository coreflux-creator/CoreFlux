...
$a('detail_for_placement scopes entries to one placement',
    preg_match("/'GET' && \\\$action === 'detail_for_placement'[\s\S]{0,1900}te\\.placement_id = :plid/", $api) === 1);
...
foreach ([
    ...
    $needle = $tid === 'timesheets-list-filter-person'
        ? "testId=\"{$tid}\""
        : "data-testid=\"{$tid}\"";
    $a("testid '{$tid}' present", str_contains($list, $needle));
}
foreach ([
...
$a('renders create-new CTA linking to timesheet week',
    str_contains($ptab, '/modules/staffing/timesheets/week?')
    && str_contains($ptab, 'weekParams.toString()'));
foreach ([
